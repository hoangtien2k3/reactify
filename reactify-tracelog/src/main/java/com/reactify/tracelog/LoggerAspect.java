...
public class LoggerAspect {
     ...
     * </p>
     */
    @Pointcut("(execution(* io.hoangtien2k3.tracelog.*.controller..*(..)) || "
            + "execution(* io.hoangtien2k3.tracelog.*.service..*(..))  ||  "
            + "execution(* io.hoangtien2k3.tracelog.*.repository..*(..)) || "
            + "execution(* io.hoangtien2k3.tracelog.*.client..*(..))) &&"
            + "!execution(* org.springframework.boot.actuate..*(..))")
    public void performancePointCut() {}

    @Pointcut("@annotation(com.reactify.tracelog.LogPerformance)")
